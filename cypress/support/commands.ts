...
Cypress.Commands.add('loginAsTestUser', () => {
    const email = getRequiredEnv('CYPRESS_USER')
    const password = getRequiredEnv('CYPRESS_USER_PASSWORD')
    ...
    cy.session(
        ...
        () => {
            ...
            cy.wait('@stytchAuthenticate').its('response.statusCode').should('be.oneOf', [200, 204])
