...
const MethodForm = forwardRef<{ submit: () => Promise<boolean> }, MethodFormProps>(
    function MethodForm(props, ref) {
        const { keyColors } = useColorMode()
        const { ingredientSections } = props
        const [draftStep, setDraftStep] = useState(createEmptyMethodStep())
        const [draftError, setDraftError] = useState<string | undefined>()
        const [editingIngredientsStepIndex, setEditingIngredientsStepIndex] = useState<
            number | null
        >(null)
        const { control, getValues, handleSubmit, reset, setValue } = useForm<MethodFormValues>({
            defaultValues: {
                steps: [],
            },
            mode: 'onTouched',
                    ...
                    handleSubmit(
                        (input) => {
                            if (!isMethodStepEmpty(draftStep)) {
                                window.alert(
                                    'Please clear the draft step or press Enter to add it before continuing.',
                                )
                                resolve(false)
                                return
                            }
        ...
        useEffect(() => {
            reset(props.initialValues ?? { steps: [] })
